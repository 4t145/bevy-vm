...
fn conditional_script_branches_per_entity() {
    let mut vm = VmWorld::load(world_path("combat.ron")).expect("配置应能成功构建世界");
    vm.tick().expect("tick 不应失败");

    let mut healthy_state = None;
    let mut weak_state = None;
    for entity in vm.query("Health") {
        let hp = number(
            &vm.get(entity, "Health", "value")
                .expect("应能读 Health.value"),
        );
        let state = string(
            &vm.get(entity, "Health", "state")
                .expect("脚本应已写入 state"),
        );
        if hp > 5.0 {
...
fn entity_reference_via_inventory_ids() {
    let mut vm = VmWorld::load(world_path("inventory.ron")).expect("配置应能成功构建世界");
    vm.tick().expect("tick 不应失败");

    let owners = vm.query("Inventory");
    ...
    let items = vm.query("Item");
    assert_eq!(items.len(), 1, "耐久耗尽的物品实体应已被 despawn，只剩剑");
    let kind = string(&vm.get(items[0], "Item", "kind").expect("应能读 Item.kind"));
    assert_eq!(kind, "sword", "存活的物品应是剑");
}

#[test]
fn infinite_loop_is_stopped_by_operation_limit() {
    let mut vm = VmWorld::load(world_path("runaway.ron")).expect("配置应能成功构建世界");
    let result = vm.tick();
...
fn compile_error_surfaces_at_construction() {
    let result = VmWorld::load(world_path("bad_syntax.ron"));
